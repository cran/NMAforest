...
#' @param study Column name identifying the study ID or label.
...
#' @param id Column name used to uniquely identify each study arm (default = "id"). If the column is not present, one will be added.
...
#'   \item{effects}{A data.frame with all effect sizes, confidence intervals, and weights.}
#'   \item{proportions}{A data.frame with proportion values.}
#'   \item{df_updated}{The input data frame with a unique \code{id} column added if not already present.}
#' }

#' @importFrom netmeta netmeta hatmatrix netsplit
#' @importFrom meta pairwise
#' @importFrom magrittr %>%
#' @importFrom dplyr filter mutate select bind_rows left_join row_number
...
#' @importFrom ggplot2 ggplot aes annotate geom_point guides geom_errorbarh geom_segment geom_text geom_vline scale_color_manual scale_y_continuous expand_limits scale_x_continuous labs annotation_custom theme_minimal theme element_text element_line element_blank
                          ...
                          study_path = TRUE)
{
  ...
  id_variants <- c("id", "ID", "Id", "iD")
  has_id <- any(names(df) %in% id_variants)

  if (!has_id){
    study_map <- data.frame(
      .study = unique(df[[study]]),
      .id = seq_along(unique(df[[study]]))
    )
    names(study_map) <- c(study, id)
    df <- left_join(df, study_map, by = study)
  }

  # Generate pairwise format
  if (sm %in% c("OR", "RR")) {
    message("Detected binary outcome: using events and sample sizes.")
    ...
    message("Detected continuous outcome: using means, SDs, and sample sizes.")
    data_long <- eval(substitute(
      pairwise(
        treat = TREAT,
        mean = MEAN,
  ...
  plot_data <- plot_data %>%
    mutate(
      ...
      size_scaled = ifelse(!is.na(weight), rescale(weight, to = c(2.5, 6)), 3)
    )
  ...
  if (length(study_rows) > 0) {
    ...
    study_ids <- data_long$id[study_rows]
    ...
    for (path_label in names(indirect_path_info)) {
      ...
      for (j in seq_len(nrow(combos))) {
        ...
        for (k in seq_along(ids)) {
          ...
          weights_k <- w[rows_k]
          this_weight <- w[data_long$id == id_k &
                             ((data_long$treat1 == t1 & data_long$treat2 == t2) |
                                (data_long$treat1 == t2 & data_long$treat2 == t1))]
  ...
  plot_object <- ggplot(plot_data, aes(y = OrderIndex)) +
    ...
    geom_point(data = subset(plot_data, !is.na(EffectSize) & Type == "Overall"),
               aes(x = EffectSize, color = Type, size = size_scaled), shape = 18) +
    geom_point(data = subset(plot_data, !is.na(EffectSize) & Type == "Direct"),
               aes(x = EffectSize, color = Type, size = size_scaled), shape = 15) +
    geom_point(data = subset(plot_data, !is.na(EffectSize) & Type == "Indirect"),
               aes(x = EffectSize, color = Type, size = size_scaled), shape = 19) +
    guides(size = "none") +
    ...
    scale_color_manual(values = c("Direct" = "blue", "Blank" = "purple", "Indirect" = "green", "Overall" = "red")) +
...
utils::globalVariables(c(
  "treat1", "treat2", ".", "UpperCI", "LowerCI", "raw_se", "weight",
  "OrderIndex", "EffectSize", "Type", "size_scaled", "xstart", "xend", "Proportion"
))
